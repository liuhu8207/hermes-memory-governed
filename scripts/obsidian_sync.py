#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Obsidian sync - full pipeline for knowledge collection and processing.
...
- Writes to knowledge/ directory (Obsidian format)
"""
...
logger = setup_logging("obsidian_sync")
...
def full_sync(days: int = 7, dashboard: bool = True) -> dict[str, Any]:
    logger.info("=== Full Obsidian Sync ===")
    results = {}
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Obsidian sync pipeline")
    ...
    parser.add_argument("--full", action="store_true", help="Run full Obsidian sync")
