...
**边界（会强制执行，不要试探）**：`memory/` 下的 `MEMORY.md` / `USER.md` /
`persona.md` 是**人工手写层，agent 不可写** —— 它们每轮作为规则注入到**所有** agent，
改它等于改自己和别人的行为准则。**文件级写入会被 PreToolUse 钩子直接拒绝**，
用 Bash 重定向绕同样会被记录。要提升某条事实的地位，走 `memory_promote.py` 或人工编辑。

- 要记**事实** → `hgm_remember`（写 L2，其他 agent 检索得到）
- 要立**规则** → 告诉用户，由用户决定是否写进 L1
  （2026-09-17 实测教训：有个 agent 被要求"记一条规则"时，直接改了 L1 文件，
   因为它找不到别的合规路径 —— 现在那条路会被拦，请走上面两条）
