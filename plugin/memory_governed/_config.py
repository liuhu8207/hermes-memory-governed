...
class RecallConfig:
    ...
    #:
    #: 标尺：这个阈值比较的对象是 ``_recall.distance_to_score`` 的输出
    #: ``1 - d/2``（对单位向量即 ``(1+cos)/2``），**不是**原始余弦；
    #: 0.76 在本标尺上约等于原始余弦 0.52。
    #:
    #: 标定依据（2026-09-16，L2=29 条）：无关查询 top1 0.731~0.741，
    #: 相关查询 top1 0.782~0.912 → 0.76 落在两者之间。
    #:
    #: ⚠️ 2026-09-22 复核：上面那条"干净切点"**只在精选语料上成立**。
    #: 同一测法用独立构造的查询集（每条相关查询的正确行都指名在库里）重测：
    #:
    #:     33 行精选集              噪声上沿 0.7686  相关下沿 0.7427  重叠 0.0259
    #:     1632 行（含 rebuild 碎片）  噪声上沿 0.8914  相关下沿 0.7506  重叠 0.1408
    #:
    #: ⇒ **两带始终重叠**，不存在既能挡住全部噪声、又能保住全部相关命中的
    #: 阈值。这里保留 0.76 只是粗筛，**不要指望它做分辨**。真正的结论是：
    #: 可用性来自**精选**（把碎片归纳成事实），不来自阈值；要改请改机制并
    #: 重新测量。
